...
def mostrar_perfil_url(request, nombre_perfil):
    ...
    d_perfil = cursor.fetchall()
    cursor.close()

    for d in range(len(d_perfil)):
        if d_perfil[d][0] == nombre_perfil:
            datos.append({
                "id": d_perfil[d][0],
                "nombre": d_perfil[d][1],
                "descrip": d_perfil[d][2],
                "departamento": d_perfil[d][3],
            })
    context = {
        'pagesize': 'A4',
